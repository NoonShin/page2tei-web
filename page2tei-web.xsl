<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xd:doc>
      <xd:desc>Metadata parameters — passed in from the web UI or derived from an uploaded METS.
         numCurr is the page sequence number (1-based), derived from filename in JS.</xd:desc>
   </xd:doc>
   <xsl:param name="numCurr"  as="xs:double" select="1"/>
   <xsl:param name="title"    as="xs:string"  select="''"/>
   <xsl:param name="author"   as="xs:string"  select="''"/>
   <xsl:param name="date"     as="xs:string"  select="''"/>
   <xsl:param name="idno"     as="xs:string"  select="''"/>
   ...
   <xd:doc>
      <xd:desc>Entry point: process a single PAGE XML file and return one TEI document.
         All metadata comes from parameters passed by the JS caller.</xd:desc>
   </xd:doc>
   <xsl:template match="/">
      <TEI>
         <xsl:text>
         ...
         <teiHeader>
            ...
            <fileDesc>
               ...
               <titleStmt>
                  ...
                  <title type="main">
                     <xsl:value-of select="if ($title != '') then $title else '[untitled]'"/>
                  </title>
                  <xsl:if test="$author != ''">
                     <xsl:text>
            </xsl:text>
   ...
   <xsl:template match="p:TextRegion | pc:TextRegion" mode="text">
      ...
      <xsl:variable name="custom" as="map(*)">
         <xsl:choose>
            <xsl:when test="@custom"><xsl:apply-templates select="@custom"/></xsl:when>
            <xsl:otherwise><xsl:map/></xsl:otherwise>
         </xsl:choose>
      </xsl:variable>
         ...
         <xsl:when test="'page-number' = $regionType and not($ab)">
            <fw type="page-number" facs="#facs_{$numCurr}_{@id}">
               <xsl:attribute name="place">
                  ...
                  <xsl:choose>
                     <xsl:when test="$verticalPosition div number(../@imageHeight) lt .33">
                        <xsl:text>top</xsl:text>
                     </xsl:when>
                     <xsl:when test="$verticalPosition div number(../@imageHeight) lt .66">
   ...
   <xsl:template match="p:TextLine | pc:TextLine">
      ...
      <xsl:if test="p:Baseline or pc:Baseline or $withoutBaseline">
         ...
         <lb facs="#facs_{$numCurr}_{@id}">
            <xsl:if test="contains(@custom, 'index:')">
   ...
   <xd:doc>
      <xd:desc>TranskribusMetadata contains the link to the image on Transkribus servers</xd:desc>
   </xd:doc>
